...
# ============================================================
st.set_page_config(page_title="Algo-Trader Pro v5.0", layout="wide")
...
with st.sidebar:
    ...
    st.write("---")
    st.subheader("Bollinger Bands Ayarları")
    bb_period = st.slider("BB Periyodu:", 10, 50, 20)
    bb_std = st.slider("BB Standart Sapma:", 1.0, 3.0, 2.0, step=0.5)

    st.write("---")
    st.subheader("MACD Ayarları")
    macd_fast = st.slider("MACD Hızlı:", 5, 20, 12)
    macd_slow = st.slider("MACD Yavaş:", 15, 40, 26)
    macd_signal = st.slider("MACD Sinyal:", 5, 15, 9)

    st.write("---")
    st.subheader("Z-Score Ayarları")
    z_period = st.slider("Z-Score Pencere:", 10, 60, 30)
    z_threshold = st.slider("Z-Score Eşik (±):", 1.0, 3.0, 2.0, step=0.1, key="k_z_thresh")

    st.write("---")
    st.subheader("OBV Ayarları")
    ...
    consensus_threshold = st.slider(
        "Konsensüs Eşik Oranı (10 = %100):", 3, 9, 5, key="k_cons_thresh",
        help="Aktif sinyallerin kaçta kaçının aynı yönde olması gerektiğini belirler. "
             "5 = %50, 6 = %60, 7 = %70 oranında çoğunluk aranır."
    )
...
if ticker:
    ...
    if not df.empty:
        ...
        min_required = max(sma_l, 30, adx_period * 3, ichi_senkou_b)
        ...
        # 3. Bollinger Bands
        df["Mid"] = close.rolling(window=bb_period).mean()
        df["Std"] = close.rolling(window=bb_period).std()
        df["Up"] = df["Mid"] + (df["Std"] * bb_std)
        df["Low_BB"] = df["Mid"] - (df["Std"] * bb_std)
        df["Sig_BB"] = np.where(close < df["Low_BB"], 1, np.where(close > df["Up"], -1, 0))

        # 4. MACD
        e_fast = close.ewm(span=macd_fast, adjust=False).mean()
        e_slow = close.ewm(span=macd_slow, adjust=False).mean()
        df["MACD"] = e_fast - e_slow
        df["MACD_S"] = df["MACD"].ewm(span=macd_signal, adjust=False).mean()
        df["Sig_MACD"] = np.where(df["MACD"] > df["MACD_S"], 1, -1)

        # 5. Mean Reversion (Z-Score)
        z_mean = close.rolling(z_period).mean()
        z_std = close.rolling(z_period).std().replace(0, np.nan)
        ...
        obv_sma_short = df["OBV"].rolling(window=obv_short, min_periods=obv_short).mean()
        obv_sma_long = df["OBV"].rolling(window=obv_long, min_periods=obv_long).mean()
        ...
        atr_ma = df["ATR"].rolling(window=atr_period, min_periods=atr_period).mean()
        ...
        df["StochRSI_K"] = df["StochRSI_K"].fillna(50).clip(0, 100)
        df["StochRSI_D"] = df["StochRSI_K"].rolling(window=stoch_d_period).mean()
        ...
        # 12. KAMA
        df["KAMA"] = calc_kama(close, period=kama_period, fast=kama_fast, slow=kama_slow)
        ...
        if is_intraday:
            ...
            signal_cols = base_signal_cols

        sig_df = df[signal_cols]
        df["AL_Count"] = (sig_df == 1).sum(axis=1)
        df["SAT_Count"] = (sig_df == -1).sum(axis=1)
        df["Valid_Count"] = (sig_df != 0).sum(axis=1)
        df["Total_Signals"] = len(signal_cols)

        dynamic_threshold = df["Valid_Count"].apply(lambda x: max(3, int(x * consensus_threshold / 10)))
        df["Consensus"] = 0
        ...
        with tab1:
            ...
            fig_rsi.add_trace(go.Scatter(
                x=df.index, y=df["RSI_MA"], name=f"RSI MA({rsi_ma_period})",
        ...
        with tab3:
            ...
            fig_adx.add_hline(y=adx_threshold, line_dash="dash", line_color="white",
                              annotation_text=f"Trend Eşiği ({adx_threshold})")
        ...
        with tab4:
            ...
            fig_obv.add_trace(go.Scatter(x=df.index, y=obv_sma_short, name=f"OBV SMA {obv_short}", line=dict(color="orange", dash="dot")))
            fig_obv.add_trace(go.Scatter(x=df.index, y=obv_sma_long, name=f"OBV SMA {obv_long}", line=dict(color="cyan", dash="dot")))
        ...
        with tab5:
            ...
            fig_stoch.add_hline(y=stoch_lower, line_dash="dash", line_color="lime",
                                annotation_text=f"Aşırı Satım ({stoch_lower})")
            fig_stoch.add_hline(y=stoch_upper, line_dash="dash", line_color="red",
        ...
        with tab9:
            fig_nw = go.Figure()
            fig_nw.add_trace(go.Scatter(x=df.index, y=close, name="Fiyat", line=dict(color="white", width=1)))
            fig_nw.add_trace(go.Scatter(x=df.index, y=df["NW_Line"], name="NW Orta", line=dict(color="gold", width=2)))
            fig_nw.add_trace(go.Scatter(x=df.index, y=df["NW_Upper"], name="NW Üst (Aşırı Alım)",
                                        line=dict(color="red", width=1, dash="dot")))
            fig_nw.add_trace(go.Scatter(x=df.index, y=df["NW_Lower"], name="NW Alt (Aşırı Satım)",
        ...
        if not np.isnan(last_sma_s) and not np.isnan(last_sma_l):
            ...
            if not last_atr_high:
                res.append(["TUT (düşük vol.)", "SMA Crossover", "ATR filtresi aktif — trend sinyali baskılandı."])
            else:
                res.append([sma_raw, "SMA Crossover", "Trend yönü tespiti."])
        else:
        ...
        if not np.isnan(last_rsi):
            if last_rsi < rsi_lower:
                ...
                rsi_dec = "TUT"
            res.append([rsi_dec, f"RSI ({rsi_period})", f"Seviye: {last_rsi:.1f}"])
        else:
            res.append(["N/A", f"RSI ({rsi_period})", "Yetersiz veri."])
        ...
        if not any(np.isnan(v) for v in [last_close, last_low_bb, last_up]):
            if last_close < last_low_bb:
                ...
                bb_dec = "TUT"
            res.append([bb_dec, f"Bollinger Bands ({bb_period},{bb_std})", "Fiyatın kanaldaki yeri."])
        else:
        ...
        if not np.isnan(last_macd) and not np.isnan(last_macd_s):
            ...
            if not last_atr_high:
                res.append(["TUT (düşük vol.)", f"MACD ({macd_fast},{macd_slow},{macd_signal})", "ATR filtresi aktif — trend sinyali baskılandı."])
            else:
                res.append([macd_raw, f"MACD ({macd_fast},{macd_slow},{macd_signal})", "Momentum durumu."])
        else:
        ...
        if not np.isnan(last_z):
            if last_z < -z_threshold:
                ...
                z_dec = "TUT"
            res.append([z_dec, f"Mean Reversion (Z={z_period})", f"Z: {last_z:.2f}"])
        else:
            res.append(["N/A", "Mean Reversion", "Yetersiz veri."])

        # 6 - OBV
        if last_obv_sig != 0 and not np.isnan(last_obv_sig):
            obv_dec = "AL" if last_obv_sig > 0 else "SAT"
            res.append([obv_dec, f"OBV ({obv_short}/{obv_long})", "Hacim trendi."])
        else:
        ...
        if not np.isnan(last_adx):
            if last_adx > adx_threshold:
                ...
                if not last_atr_high:
                    res.append(["TUT (düşük vol.)", "ADX", f"ADX: {last_adx:.1f} — ATR filtresi aktif."])
                else:
                    res.append([adx_raw, "ADX", f"ADX: {last_adx:.1f} (Güçlü trend)"])
            else:
                res.append(["TUT", "ADX", f"ADX: {last_adx:.1f} (Zayıf trend)"])
        else:
        ...
        if is_intraday:
            ...
            if not np.isnan(last_vwap):
                if last_vwap_sig == 1:
                    ...
                    vwap_dec = "TUT"
                res.append([vwap_dec, "VWAP", f"VWAP: {last_vwap:.2f} | bant: ±%{vwap_band_pct:.2f}"])
            else:
        ...
        if not np.isnan(last_stoch_k):
            if last_stoch_k < stoch_lower:
                ...
                stoch_dec = "TUT"
            res.append([stoch_dec, f"Stoch RSI ({stoch_rsi_period})", f"%K: {last_stoch_k:.1f}"])
        else:
        ...
        if not np.isnan(last_kama):
            ...
            if not last_atr_high:
                res.append(["TUT (düşük vol.)", f"KAMA ({kama_period},{kama_fast},{kama_slow})", f"KAMA: {last_kama:.2f} — ATR filtresi aktif."])
            else:
                res.append([kama_raw, f"KAMA ({kama_period},{kama_fast},{kama_slow})", f"KAMA: {last_kama:.2f}"])
        else:
        ...
        if not np.isnan(last_st):
            ...
            if not last_atr_high:
                res.append(["TUT (düşük vol.)", f"SuperTrend ({st_period},{st_multiplier})", f"Seviye: {last_st:.2f} — ATR filtresi aktif."])
            else:
                res.append([st_raw, f"SuperTrend ({st_period},{st_multiplier})", f"Seviye: {last_st:.2f}"])
        else:
        ...
        if not np.isnan(last_lrc_mid):
            if last_lrc_sig == 1:
                ...
                lrc_dec = "TUT"
            res.append([lrc_dec, f"LR Channel ({lrc_period})", f"Orta: {last_lrc_mid:.2f}"])
        else:
            res.append(["N/A", "LR Channel", "Yetersiz veri."])

        # ATR Filtre (bilgi)
        last_atr = safe_scalar(last["ATR"])
        last_atr_ma = safe_scalar(atr_ma.iloc[-1])
        if not np.isnan(last_atr):
            vol_status = "Yüksek ↑" if last_atr_high else "Düşük ↓"
            atr_ma_str = f"{last_atr_ma:.2f}" if not np.isnan(last_atr_ma) else "N/A"
            res.append(["BİLGİ", "ATR Filtre", f"Volatilite: {vol_status} | ATR: {last_atr:.2f} | ATR MA({atr_period}): {atr_ma_str}"])
        else:
        ...
        if not np.isnan(last_nw):
            if last_close > last_nw_up:
                nw_note = "Üst zarfın üstünde (aşırı alım bölgesi)"
            elif last_close < last_nw_lo:
                nw_note = "Alt zarfın altında (aşırı satım bölgesi)"
            else:
        ...
        last_dynamic_threshold = max(3, int(df["Valid_Count"].iloc[-1] * consensus_threshold / 10))

        if al_count >= last_dynamic_threshold:
            consensus_label = "🟢 AL"
        elif sat_count >= last_dynamic_threshold:
            consensus_label = "🔴 SAT"
        else:
        ...
        st.caption(
            ...
            "Backtest overfitting riski içerir."
        )
        ...
        if trades:
            ...
            m10.metric("En İyi / En Kötü", f"{best_trade:.2f}% / {worst_trade:.2f}%")
        ...
        if run_optimization:
            ...
            def run_backtest_with_params(data_slice, close_slice, p_sma_s, p_sma_l,
                ...
                atr_opt = tr_opt.ewm(alpha=1.0 / atr_period, min_periods=atr_period, adjust=False).mean()
                atr_ma_opt = atr_opt.rolling(window=atr_period, min_periods=atr_period).mean()
                ...
                delta_c = c.diff()
                gain = delta_c.where(delta_c > 0, 0.0).rolling(window=rsi_period).mean()
                loss_s = (-delta_c.where(delta_c < 0, 0.0)).rolling(window=rsi_period).mean()
                rs = gain / loss_s.replace(0, np.nan)
                rsi = 100 - (100 / (1 + rs))
                sig_rsi = np.where(rsi < p_rsi_lower, 1, np.where(rsi > p_rsi_upper, -1, 0))

                mid = c.rolling(window=bb_period).mean()
                std = c.rolling(window=bb_period).std()
                up = mid + (std * bb_std)
                lo_bb = mid - (std * bb_std)
                sig_bb = np.where(c < lo_bb, 1, np.where(c > up, -1, 0))

                e_f = c.ewm(span=macd_fast, adjust=False).mean()
                e_s = c.ewm(span=macd_slow, adjust=False).mean()
                macd = e_f - e_s
                macd_s = macd.ewm(span=macd_signal, adjust=False).mean()
                sig_macd = np.where(macd > macd_s, 1, -1)
                sig_macd = np.where(atr_high_opt | (sig_macd == 0), sig_macd, 0)

                z_m = c.rolling(z_period).mean()
                z_s = c.rolling(z_period).std().replace(0, np.nan)
                z_val = (c - z_m) / z_s
                sig_z = np.where(z_val < -p_z_thresh, 1, np.where(z_val > p_z_thresh, -1, 0))

                obv_sign = np.sign(c.diff()).fillna(0)
                obv = (v * obv_sign).cumsum()
                obv_s = obv.rolling(window=obv_short, min_periods=obv_short).mean()
                obv_l = obv.rolling(window=obv_long, min_periods=obv_long).mean()
                sig_obv = np.where(obv_s > obv_l, 1, -1)
                sig_obv = np.where(obv_s.isna() | obv_l.isna(), 0, sig_obv)

                adx_v, pdi, mdi = calc_adx(h, l, c, period=adx_period)
                sig_adx = np.where(adx_v > adx_threshold, np.where(pdi > mdi, 1, -1), 0)
                ...
                sig_stoch = np.where(stoch_k < stoch_lower, 1, np.where(stoch_k > stoch_upper, -1, 0))

                tenkan = (h.rolling(window=ichi_tenkan).max() + l.rolling(window=ichi_tenkan).min()) / 2
                kijun = (h.rolling(window=ichi_kijun).max() + l.rolling(window=ichi_kijun).min()) / 2
                senkou_a = ((tenkan + kijun) / 2).shift(ichi_kijun)
                senkou_b = ((h.rolling(window=ichi_senkou_b).max() + l.rolling(window=ichi_senkou_b).min()) / 2).shift(ichi_kijun)
                cloud_top = pd.concat([senkou_a, senkou_b], axis=1).max(axis=1)
                ...
                kama_s = calc_kama(c, period=kama_period, fast=kama_fast, slow=kama_slow)
                ...
                _, st_dir, _, _ = calc_supertrend(h, l, c, period=st_period, multiplier=st_multiplier)
                sig_st = st_dir.values
                sig_st = np.where(atr_high_opt | (sig_st == 0), sig_st, 0)

                lrc_mid, lrc_up, lrc_lo = calc_linear_regression_channel(c, period=lrc_period, std_mult=lrc_std_mult)
                ...
                valid_cnt = (all_sigs != 0).sum(axis=1)
                dyn_thresh = np.maximum(3, (valid_cnt * 0.5).astype(int))
                consensus = np.zeros(len(c))
                consensus[al_cnt >= dyn_thresh] = 1
                consensus[sat_cnt >= dyn_thresh] = -1
            ...
            if not windows:
                ...
                for w_idx, win in enumerate(windows):
                    ...
                    for idx, combo in enumerate(all_combos):
                        ...
                        if len(c_train) < max(p["sma_l"], ichi_senkou_b) + 30:
                            work_done += 1
                            continue
                ...
                if best_combo_key is not None:
                    ...
                    if positive_windows == total_windows:
                        ...
                        st.warning(f"⚠️ {total_windows} pencereden {positive_windows}'inde pozitif getiri — kısmen güvenilir.")
                    else:
